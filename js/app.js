...
function renderDashboard(){
 ...
 expense=tx.filter(t=>t.type==="expense")
   .reduce((a,t)=>a+Number(t.amount),0);
 ...
 $("balance").textContent=money(allIncome-allExpense);
 ...
 $("greeting").textContent=
   "Halo, "+(state.user.email?.split("@")[0]||"Finova")+" 👋";
...
function renderBudgetPreview(){
 const m=monthISO(),
 bud=state.budgets.filter(
   b=>String(b.month).startsWith(m)
 );

 $("budgetPreview").innerHTML=
   bud.length
   ?bud.slice(0,4).map(b=>budgetHTML(b)).join("")
   :`<div class="muted">Belum ada anggaran bulan ini.</div>`;
}

function spentForBudget(b){
 return monthTx(String(b.month).slice(0,7))
   .filter(
     t=>
       t.type==="expense"&&
       (
...
function budgetHTML(b){
 ...
 return `<div class="budget-item">
   <div class="budget-top">
     <b>${esc(b.categories?.icon||"📦")} ${esc(b.categories?.name||"Kategori")}</b>
...
function renderBudgetList(){
 $("budgetList").innerHTML=
   state.budgets
     .filter(b=>String(b.month).startsWith(monthISO()))
     .map(
       b=>
         budgetHTML(b)+
         `<div>
           <button class="link-btn" onclick="editBudget('${b.id}')">Edit</button>
           <button class="link-btn danger-text" onclick="deleteBudget('${b.id}')">Hapus</button>
         </div>`
     )
     .join("")||
...
function renderGoalPreview(){
 $("goalPreview").innerHTML=
   state.goals.slice(0,3).map(goalHTML).join("")||
...
function goalHTML(g){
 ...
 return `<div class="goal-card">
   ...
   ${
     g.deadline
     ?`<small class="muted">
        Target ${new Date(g.deadline).toLocaleDateString("id-ID")}
       </small>`
     :""
   }
 </div>`
 ...
 $("goalList").innerHTML=
   state.goals.map(
     g=>
       goalHTML(g)+
       `<div>
         <button class="link-btn" onclick="editGoal('${g.id}')">Edit</button>
         <button class="link-btn danger-text" onclick="deleteGoal('${g.id}')">Hapus</button>
       </div>`
   ).join("")||
   `<div class="muted">Belum ada target.</div>`
}
...
function renderTransactions(){
 ...
 const selectedCategory=
   state.categories.find(c=>c.id===cat);

 const selectedCategoryName=
   selectedCategory?.name||"";

 let tx=state.transactions.filter(
   t=>
     (
       !q||
 ...
 $("transactionList").innerHTML=
   tx.map(t=>{
     const c=state.categories.find(
       x=>x.name===t.category&&x.type===t.type
     );
   ...
   .join("")||
   `<div class="muted">Tidak ada transaksi.</div>`;
}

["searchTx","filterType","filterCategory","filterMonth"]
 .forEach(
   id=>$(id).addEventListener("input",renderTransactions)
 );
...
function txForm(t={}){
 ...
 const selectedId=
   state.categories.find(
     c=>c.name===t.category&&c.type===type
   )?.id||"";
 ...
 return `<form id="txForm">
   ...
   <div class="form-actions">
     ...
     <button type="button" class="secondary" id="cancelForm">
       Batal
     </button>
   </div>
 </form>`;
...
function openTx(t){
 openModal(
   t?"Edit transaksi":"Tambah transaksi",
   txForm(t)
 );

 $("fType").onchange=()=>{
   $("fCategory").innerHTML=
     categoryOptions($("fType").value)
 };
 ...
 $("txForm").onsubmit=async e=>{
   ...
   cid=$("fCategory").value,
   c=state.categories.find(x=>x.id===cid);
...
function openBudget(b){
 openModal(
   b?"Edit anggaran":"Tambah anggaran",
   budgetForm(b)
 );

 $("cancelBudget").onclick=closeModal;
...
function openGoal(g){
 openModal(
   g?"Edit target":"Tambah target",
   goalForm(g)
 );

 $("cancelGoal").onclick=closeModal;
 ...
 $("goalForm").onsubmit=async e=>{
   ...
   const p={
     ...
     icon:$("gIcon").value||"🎯",
     target_amount:Number($("gTarget").value),
     current_amount:Number($("gCurrent").value||0),
...
function renderHealth(){
 ...
 const saving=income
   ?Math.max(0,(income-expense)/income)
   :0,

 bud=state.budgets
   .filter(b=>String(b.month).startsWith(monthISO())),

 avgBudget=bud.length
   ?bud.reduce(
      (a,b)=>a+Math.min(
        1,
        spentForBudget(b)/Number(b.amount)
      ),
      0
    )/bud.length
   :0,
 ...
 $("healthBadge").textContent=`Health ${score}/100`;
 $("healthMeter").style.width=Math.min(100,score)+"%";
 $("healthText").textContent=score+"/100";
...
function renderReports(){
 const m=$("reportMonth").value||state.reportMonth;
...
$("reportRefresh").onclick=renderReports;
